...
export default function Orders(){
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (response?.success && Array.isArray(response.data)) {
          ...
          const transformedOrders = response.data.map(order => ({
            ...
            items: [] // Backend will provide order items separately if needed
          }))
    ...
    // Listen for admin updates and refetch when orders change
    const onUpdated = () => { fetchUserOrders() }
    window.addEventListener('ordersUpdated', onUpdated)
    return () => { window.removeEventListener('ordersUpdated', onUpdated) }
  }, []) // Empty dependency array: fetch only on mount

  // Auto-update when new order is placed in CartContext
  useEffect(() => {
    if (contextOrders.length > orders.length) {
      setOrders(prev => {
        const newOrders = contextOrders.map(o => ({
          orderId: o.orderId,
      ...
      <div className="page-container orders-page" style={{paddingTop:18}}>
        <div className="section-head" style={{alignItems:'center'}}>
          <div>
            ...
            <div className="muted" style={{marginTop:6}}>{loading ? 'Loading...' : `${orders.length} order${orders.length!==1? 's':''}`}</div>
          </div>
        </div>

        {error && (
          <div className="card" style={{marginTop:18, padding:16, background:'#fee2e2', border:'1px solid #fecaca', borderRadius:8, color:'#991b1b'}}>
            <strong>Error:</strong> {error}
          </div>
        )}

        {loading ? (
          <div className="card orders-empty" style={{marginTop:18, textAlign:'center', padding:32}}>
            <div style={{fontSize:16, color:'#6b7280'}}>Loading your orders...</div>
          </div>
        ) : orders.length === 0 ? (
