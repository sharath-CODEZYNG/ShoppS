...
export default function Navbar(){
  ...
  // Calculate cart count from CartContext (single source of truth)
  const cartCount = Array.isArray(cart) ? cart.reduce((sum, item) => sum + (Number(item.quantity) || 0), 0) : 0

  useEffect(()=>{
    // Read `user` from localStorage (persisted auth)
    ...
    function onUserChanged(){
      // custom event dispatched by other components after profile save
