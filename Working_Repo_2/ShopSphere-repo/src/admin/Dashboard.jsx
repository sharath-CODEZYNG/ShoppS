...
export default function Dashboard(){
  const totals = useMemo(()=>({ users:1234, orders:2345, products:60, revenue: 1250000 }), [])

  const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']
  const monthly = [80000,95000,87000,120000,110000,98000,125000,135000,128000,140000,155000,165000]
    ...
    <div>
      ...
      <div className="admin-cards" style={{marginTop:12}}>
        ...
        <div className="admin-card">
          ...
          <div style={{fontSize:22,fontWeight:700,marginTop:6}}>{totals.products}</div>
          <div style={{fontSize:12,color:'#999',marginTop:6}}>Active product listings</div>
        </div>
        ...
        <div className="admin-card">
          ...
          <div style={{fontSize:12,color:'#999',marginTop:6}}>Year-to-date revenue</div>
        </div>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 420px',gap:12,marginTop:18}}>
        ...
        <div className="card">
          ...
          <table className="table" style={{marginTop:12}}>
            ...
            <tbody>
              {recentUsers.map(u=> (
                <tr key={u.email}><td>{u.name}</td><td>{u.email}</td><td>{u.created}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
