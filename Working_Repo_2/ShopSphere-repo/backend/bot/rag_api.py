...
- If no matching product exists, reply EXACTLY:
  No matching product found in our catalogue.
- If there are CURRENTLY ACTIVE PRODUCTS on the screen, you are strictly forbidden from suggesting ANY of those exact products again.
- if product is not present do not recommend other products.
...
def rerank_docs_by_query(docs, query: str, query_vector):
    ...
    def score(doc):
        ...
        # semantic similarity
        doc_vector = get_embedding(doc.page_content)
...
def agentic_route_query(query: str, has_active_context: bool) -> str:
    ...
    1. "goal": The user wants to achieve a task, prepare for an event, cook a recipe, or needs a list of items (e.g., "I want to bake a cake", "I am sending my brother to school", "what do I need for pasta"). Do NOT use this if they mention days or budgets or plan.if the user is expressing his feelings, what he feels use this.
    2. "follow_up": The user is asking about an item currently in context using pronouns ("it", "this", "that") OR asking for related/similar items,other related products without specifying a new product name (e.g., "what is its price", "is it in stock?", "show me related products", "what else is similar?"). NOTE: Only select this if has_context is True.
